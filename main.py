...
def extract_topic(original_title: str) -> str:
    """
    Extract topic name from the raw title line (before ':' URL separator).
    Handles 4 title formats:

    1. [Arithmetic] Class-03 | Percentage          → Arithmetic
    2. [by Aman Sir] Class -3 | English Basics ...  → English Basics
    3. Class - 03 || Percentage by Gagan sir        → Percentage
    4. Profit_Loss_Class_3                          → Profit Loss

    NOTE: Trailing lecture/class number suffixes are always stripped so that
    "English Pronoun lec -1" and "English Pronoun lec -2" both yield "English Pronoun".
    """
    title = original_title.strip()

    # --- Rule 1 & 2 : title begins with [...] or (...) ---
    bracket_match = re.match(r'^[\[\(]([^\]\)]+)[\]\)]', title)
    if bracket_match:
        bracket_content = bracket_match.group(1).strip()
        # If bracket says "by XYZ" it is a teacher tag, not a topic
        if bracket_content.lower().startswith('by '):
            # Topic comes after the first '|'
            parts = re.split(r'\|', title, maxsplit=1)
            if len(parts) > 1:
                rest = parts[1].strip()
                rest = re.split(r'\|\|', rest)[0].strip()          # drop || part
                ...
                rest = _strip_lecture_suffix(rest)   # ← strip trailing lec/class N
                return rest if rest else bracket_content
            return bracket_content
        else:
            return bracket_content

    # --- Rule 3 : "Class - 03 || Percentage by Gagan sir" ---
    if '||' in title:
        ...
        topic = _strip_lecture_suffix(topic)         # ← strip trailing lec/class N
        if topic:
            return topic

    # --- Rule 4 : "Profit_Loss_Class_3" (snake_case_ClassN) ---
    ...
    fallback = _strip_lecture_suffix(fallback)       # ← strip trailing lec/class N
...
photologo = 'https://ibb.co/m5HCqdzN' #https://ibb.co/m5HCqdzN
photoyt = 'https://ibb.co/m5HCqdzN' #https://ibb.co/m5HCqdzN
photocp = 'https://ibb.co/m5HCqdzN'
photozip = 'https://ibb.co/m5HCqdzN'
...
image_urls = [
    "https://ibb.co/m5HCqdzN",
    "https://ibb.co/m5HCqdzN",
    "https://ibb.co/m5HCqdzN",
    # Add more image URLs as needed
]

# 🖼️ Local images used for topic banners (sent + pinned when a new topic starts)
TOPIC_IMAGE_FILES = [
    "topic_images/32156.jpg",
    "topic_images/32154.jpg",
    "topic_images/32152.jpg",
    "topic_images/32150.jpg",
    "topic_images/32146.jpg",
    "topic_images/32144.jpg",
    "topic_images/32160.jpg",
    "topic_images/32158.jpg",
]
    ...
    try:
        if m.chat.type == "channel":
            ...
            if not is_authorized:
                await m.reply_photo(
                    photo=photologo,
                    caption="**Mʏ Nᴀᴍᴇ [DRM Wɪᴢᴀʀᴅ 🦋](https://t.me/kesav503)\n\nYᴏᴜ ᴅᴏɴ'ᴛ ʜᴀᴠᴇ ᴀᴄᴄᴇꜱꜱ ᴛᴏ ᴜꜱᴇ ᴛʜɪꜱ ʙᴏᴛ\nCᴏɴᴛᴀᴄᴛ [𝐈𝐓'𝐬𝐆𝐎𝐋𝐔.™®](https://t.me/kesav503) ғᴏʀ ᴀᴄᴄᴇꜱꜱ**",
                    reply_markup=InlineKeyboardMarkup([
    [
        InlineKeyboardButton("𝐈𝐓'𝐬𝐆𝐎𝐋𝐔.™®", url="https://t.me/kesav503")
    ],
    [
...
async def unauthorized_handler(client, message: Message):
    await message.reply(
        "<b>Mʏ Nᴀᴍᴇ [DRM Wɪᴢᴀʀᴅ 🦋](https://t.me/kesav503)</b>\n\n"
...
async def back_to_start_callback(client, callback_query: CallbackQuery):
    ...
    await callback_query.message.edit_media(
        ...
        reply_markup=InlineKeyboardMarkup([
            [
                InlineKeyboardButton("𝐈𝐓'𝐬𝐆𝐎𝐋𝐔.™®", url="https://t.me/RajKesav5")
            ],
            [
                InlineKeyboardButton("ғᴇᴀᴛᴜʀᴇꜱ 🪔", callback_data="features"),
                InlineKeyboardButton("ᴅᴇᴛᴀɪʟꜱ 🦋", callback_data="details")
